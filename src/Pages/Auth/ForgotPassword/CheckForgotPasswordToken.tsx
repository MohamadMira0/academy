...
import { ForgotTokenValidation } from '../../../Validation/auth/ForgotPassword';
...
const CheckForgotPasswordToken = () => {
  ...
  const handleSubmitForgot = async (
    ...
    try {
      console.log('check forgot code');
      setErr('');
      const res = await axios.post(
        `${base_url_student}/auth/check-forgot-password-token`,
        values,
      );
      console.log(res);
      nav('/reset-password');
      ...
      if (err.response.status === 400) {
        setErr('code is incorrect | try again');
      } else {
          ...
          <div className="bg-white rounded-lg shadow dark:bg-gray-700 w-11/12 md:w-3/4 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            <div className="p-4 md:p-5 space-y-4">
              <Formik
                ...
                validationSchema={ForgotTokenValidation}
